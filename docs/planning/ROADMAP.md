...
**MVP limitation decisions:** CLI/MCP support one-off linked transfers. Recurring transfer rules remain deferred until recurring materialization carries destination-account support.
...
| Subscription list page                  | Future  | Browser UI removed from v1 scope          |
...
**MVP limitation decision:** Subscription data is supported in the local schema, bill forecasting, and CLI/MCP analytics. Browser subscription management and any external Subby import/sync are future scope, not part of v1.
