...
export async function runTauriMigrations(db: TauriDatabase): Promise<void> {
  ...
  if (!applied.has('019_financial_semantics')) {
    await ensureTableColumn(
      ...
      'accounts',
      'account_mode',
      `TEXT NOT NULL DEFAULT 'transactional' CHECK (account_mode IN ('transactional', 'snapshot_only'))`
    )
    await db.execute(`
    ...
    await ensureTableColumn(
      ...
      'transactions',
      'ledger_treatment',
      `TEXT NOT NULL DEFAULT 'normal' CHECK (ledger_treatment IN ('normal', 'staged_no_balance_impact'))`
    )
    await ensureTableColumn(
      db,
      'transactions',
      'reporting_treatment',
      `TEXT NOT NULL DEFAULT 'normal' CHECK (reporting_treatment IN ('normal', 'exclude_from_cashflow'))`
    )
    await ensureTableColumn(
      db,
      'transactions',
      'transaction_kind',
      `TEXT NOT NULL DEFAULT 'standard' CHECK (transaction_kind IN ('standard', 'reconciliation_bridge', 'archived_transfer_mirror'))`
    )
    await ensureTableColumn(db, 'transactions', 'staging_batch_id', 'TEXT')
    await ensureTableColumn(
      db,
      'transactions',
      'reconciliation_id',
      'TEXT REFERENCES account_reconciliations(id) ON DELETE RESTRICT'
    )
    await ensureTableColumn(
      db,
      'transactions',
      'matched_transaction_id',
      'TEXT REFERENCES transactions(id) ON DELETE RESTRICT'
    )
    await ensureTableColumn(db, 'transactions', 'is_archived', 'INTEGER NOT NULL DEFAULT 0')
    await db.execute(`
    ...
    `)
    await db.execute(`CREATE INDEX IF NOT EXISTS idx_accounts_mode ON accounts(account_mode)`)
    await db.execute(
    ...
    await db.execute(
      `CREATE INDEX IF NOT EXISTS idx_reconciliations_account_date ON account_reconciliations(account_id, reconciliation_date)`
    )
    await db.execute(
      `CREATE INDEX IF NOT EXISTS idx_reconciliations_batch ON account_reconciliations(account_id, staging_batch_id)`
    )
    await db.execute(
    ...
    await db.execute(
      `CREATE UNIQUE INDEX IF NOT EXISTS idx_transactions_reconciliation_unique ON transactions(reconciliation_id) WHERE reconciliation_id IS NOT NULL`
    )
    await db.execute(
      `CREATE UNIQUE INDEX IF NOT EXISTS idx_transactions_matched_unique ON transactions(matched_transaction_id) WHERE matched_transaction_id IS NOT NULL`
    )
    await db.execute(
      `CREATE UNIQUE INDEX IF NOT EXISTS idx_reconciliations_batch_unique ON account_reconciliations(account_id, staging_batch_id) WHERE staging_batch_id IS NOT NULL`
    )
    await db.execute(
      `CREATE UNIQUE INDEX IF NOT EXISTS idx_reconciliations_adjustment_unique ON account_reconciliations(adjustment_transaction_id) WHERE adjustment_transaction_id IS NOT NULL`
    )
    await db.execute(
      `CREATE UNIQUE INDEX IF NOT EXISTS idx_receivables_transaction_unique ON receivables(matched_transaction_id) WHERE matched_transaction_id IS NOT NULL`
    )
    await db.execute(`
    ...
    `)
    await db.execute(
      "INSERT OR IGNORE INTO _migrations (id, name) VALUES (19, '019_financial_semantics')"
    )
