  ...
  return {
    query: async <T>(sql: string, bindValues: unknown[] = []): Promise<T[]> => {
      ...
      if (sql.includes('FROM transactions')) {
        ...
        if (sql.includes('WHERE import_fingerprint = ?')) {
          const count = working.transactions.filter(
            (transaction) => transaction.import_fingerprint === bindValues[0]
          ).length
        ...
        const count = working.transactions.filter(
          (transaction) =>
            ...
            (transaction.import_fingerprint === null ||
              transaction.import_fingerprint === undefined ||
              transaction.import_fingerprint.trim() === '') &&
...
describe('importStatementFile', () => {
  ...
  it('deduplicates OFX rows by their persisted external identity', async () => {
    mocks.parseStatement.mockReturnValue([
      makeParsed({ externalId: 'bank-row-7', description: 'Original description' }),
    ])

    const first = await importStatementFile(statementFile(), 'account-1')
    expect(first).toEqual({ imported: 1, skipped: 0, errors: [] })

    mocks.parseStatement.mockReturnValue([
      makeParsed({
        externalId: 'bank-row-7',
        description: 'Bank changed the description',
        date: '2026-03-15',
      }),
    ])
    const second = await importStatementFile(statementFile(), 'account-1')

    expect(second).toEqual({ imported: 0, skipped: 1, errors: [] })
    expect(database.transactions).toHaveLength(1)
    expect(database.transactions[0].import_source).toBe('statement:ofx')
    expect(database.transactions[0].import_external_id).toBe('bank-row-7')
  })
