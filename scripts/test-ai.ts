...
/**
 * CLI test suite for Ivy (Shikin AI) with real LLM calls + mocked DB.
 *
 * Usage:
...
const provider = (process.env.PROVIDER || 'openrouter') as
  | 'openrouter'
  | 'openai'
  | 'anthropic'
  | 'ollama'
...
function fromCentavos(n: number) {
  return n / 100
}
function toCentavos(n: number) {
...
const tools = {
  addTransaction: tool({
    ...
    execute: async ({ amount, type, description, category, date }) => {
      ...
      return {
        ...
        message: `Added ${type}: $${amount.toFixed(2)} for "${description}"`,
      }
    },
  }),
  ...
  getSpendingSummary: tool({
    ...
    execute: async ({ period }) => {
      ...
      const byCategory = MOCK_CATEGORIES.map((c) => {
        ...
        return {
          category: c.name,
          amount: fromCentavos(catExp),
          percentage: totalExp > 0 ? Math.round((catExp / totalExp) * 100) : 0,
        }
      }).filter((c) => c.amount > 0)
      return {
        period: { start: '2026-02-01', end: '2026-02-28', label: period },
        totalExpenses: fromCentavos(totalExp),
        totalIncome: fromCentavos(income),
        netSavings: fromCentavos(income - totalExp),
        byCategory,
        message: `Total spending: $${fromCentavos(totalExp).toFixed(2)}`,
      }
    },
  }),
  ...
  deleteTransaction: tool({
    ...
    execute: async ({ transactionId }) => {
      ...
      return {
        success: true,
        message: `Deleted ${tx.type}: $${fromCentavos(tx.amount).toFixed(2)} "${tx.description}"`,
      }
    },
  }),
  ...
  queryTransactions: tool({
    ...
    execute: async ({ type, startDate, endDate, search, limit }) => {
      ...
      if (search)
        results = results.filter((t) => t.description.toLowerCase().includes(search.toLowerCase()))
      ...
      return {
        transactions: results.map((t) => ({
          ...
          type: t.type,
          category: MOCK_CATEGORIES.find((c) => c.id === t.category_id)?.name || 'Uncategorized',
          account: MOCK_ACCOUNTS.find((a) => a.id === t.account_id)?.name || 'Unknown',
          date: t.date,
          notes: t.notes,
        })),
  ...
  listAccounts: tool({
    ...
    execute: async ({ type }) => {
      ...
      return {
        ...
        message: `Found ${accs.length} account(s).`,
      }
    },
  }),
  ...
  createAccount: tool({
    ...
    execute: async ({ name, type, currency, balance }) => {
      ...
      return {
        success: true,
        account: { id, name, type, currency, balance },
        message: `Created ${type} account "${name}" with balance $${balance.toFixed(2)}`,
      }
    },
  }),
  ...
  listCategories: tool({
    ...
    execute: async ({ type }) => {
      ...
      return {
        ...
        message: `Found ${cats.length} categor${cats.length !== 1 ? 'ies' : 'y'}.`,
      }
    },
  }),
  ...
  getBalanceOverview: tool({
    ...
    execute: async () => {
      ...
      const currentExpenses = MOCK_TRANSACTIONS.filter(
        (t) => t.type === 'expense' && t.date >= '2026-02-01'
      ).reduce((s, t) => s + t.amount, 0)
      const currentIncome = MOCK_TRANSACTIONS.filter(
        (t) => t.type === 'income' && t.date >= '2026-02-01'
      ).reduce((s, t) => s + t.amount, 0)
      const prevExpenses = MOCK_TRANSACTIONS.filter(
        (t) => t.type === 'expense' && t.date >= '2026-01-01' && t.date < '2026-02-01'
      ).reduce((s, t) => s + t.amount, 0)
      const prevIncome = MOCK_TRANSACTIONS.filter(
        (t) => t.type === 'income' && t.date >= '2026-01-01' && t.date < '2026-02-01'
      ).reduce((s, t) => s + t.amount, 0)
      ...
      return {
        ...
        monthlyChange: {
          ...
          previous: fromCentavos(prevNet),
          trend: currentNet > prevNet ? 'up' : currentNet < prevNet ? 'down' : 'stable',
        },
    ...
    execute: async ({ months }) => {
      ...
      return {
        months: [
          {
            month: '2026-01',
            totalExpenses: fromCentavos(14800),
            totalIncome: fromCentavos(500000),
            net: fromCentavos(485200),
            topCategories: [
              { category: 'Food & Dining', amount: fromCentavos(9800) },
              { category: 'Transportation', amount: fromCentavos(5000) },
            ],
          },
          {
            month: '2026-02',
            totalExpenses: fromCentavos(17951),
            totalIncome: fromCentavos(500000),
            net: fromCentavos(482049),
            topCategories: [
              { category: 'Shopping', amount: fromCentavos(8999) },
              { category: 'Food & Dining', amount: fromCentavos(5450) },
              { category: 'Transportation', amount: fromCentavos(3500) },
            ],
          },
        ],
        trends: [
  ...
  recallMemories: tool({
    ...
    execute: async ({ search, category, limit }) => {
      ...
      if (search)
        results = results.filter((m) => m.content.toLowerCase().includes(search.toLowerCase()))
      ...
      return {
        ...
        message:
          results.length === 0
            ? 'No memories found.'
            : `Found ${results.length} memory${results.length !== 1 ? 'ies' : ''}.`,
      }
    },
  }),
    ...
    execute: async ({ accountId }) => {
      ...
      if (hasTx) {
        ...
        return {
          ...
          message: `Archived account "${MOCK_ACCOUNTS[idx].name}".`,
        }
      }
      ...
      return {
        ...
        message: `Deleted account "${acc.name}".`,
      }
    },
  }),
  ...
  getCreditCardStatus: tool({
    description:
      'Get credit card status including balance, limit, utilization, and upcoming dates.',
    inputSchema: zodSchema(
      z.object({
        accountId: z.string().optional(),
      })
    ...
    execute: async ({ accountId }) => {
      ...
      const statuses = cards.map((c) => ({
        ...
        utilizationPercent: c.credit_limit
          ? Math.round((Math.abs(c.balance) / c.credit_limit) * 100)
          : null,
  ...
  createBudget: tool({
    ...
    execute: async ({ categoryName, amount, period, name }) => {
      ...
      return {
        success: true,
        budget: { id, name: budgetName, amount, period },
        message: `Created ${period} budget "${budgetName}" for $${amount.toFixed(2)}.`,
      }
    },
  }),
    ...
    execute: async ({ categoryId }) => {
      ...
      const statuses = budgets.map((b) => {
        const spent = MOCK_TRANSACTIONS.filter(
          (t) => t.type === 'expense' && t.category_id === b.category_id && t.date >= '2026-02-01'
        ).reduce((s, t) => s + t.amount, 0)
        return {
          id: b.id,
          name: b.name,
          budgetAmount: fromCentavos(b.amount),
          spentAmount: fromCentavos(spent),
          remaining: fromCentavos(b.amount - spent),
    ...
    execute: async ({ activeOnly }) => {
      ...
      return {
        ...
        summary: {
          ...
          estimatedYearly: totalMonthly * 12,
        },
        message: `${subs.length} subscription(s). Monthly: $${totalMonthly.toFixed(2)}.`,
      }
    },
  }),
  ...
  getSubscriptionSpending: tool({
    ...
    execute: async () => {
      ...
      return {
        ...
        message: `${MOCK_SUBSCRIPTIONS.length} subscriptions. Monthly: $${totalMonthly.toFixed(2)}.`,
      }
    },
  }),
  ...
  getNetWorth: tool({
    ...
    inputSchema: zodSchema(z.object({})),
    execute: async () => {
      const assets = MOCK_ACCOUNTS.filter((a) => a.type !== 'credit_card' && !a.is_archived).reduce(
        (s, a) => s + a.balance,
        0
      )
      const liabilities = MOCK_ACCOUNTS.filter(
        (a) => a.type === 'credit_card' && !a.is_archived
      ).reduce((s, a) => s + Math.abs(a.balance), 0)
      const investments = MOCK_INVESTMENTS.reduce(
        (s, i) => s + i.shares * fromCentavos(i.avg_cost_basis),
        0
      )
      ...
      return {
        success: true,
        netWorth,
        totalAssets: fromCentavos(assets) + investments,
        totalLiabilities: fromCentavos(liabilities),
    ...
    execute: async ({ action, investmentId, name, symbol, type, shares, avgCost }) => {
      if (action === 'add') {
        ...
        MOCK_INVESTMENTS.push({
          ...
          type: type || 'stock',
          shares: shares || 0,
          avg_cost_basis: avgCost ? toCentavos(avgCost) : 0,
          currency: 'USD',
          notes: null,
          created_at: new Date().toISOString(),
          updated_at: new Date().toISOString(),
        ...
        return {
          success: true,
          investment: { id, name, symbol },
          message: `Added investment: ${name} (${symbol}).`,
        }
      }
  ...
  getUpcomingBills: tool({
    ...
    execute: async ({ daysAhead }) => {
      ...
      for (const acc of MOCK_ACCOUNTS.filter(
        (a) => a.type === 'credit_card' && a.payment_due_day
      )) {
        bills.push({
          name: `${acc.name} payment`,
          amount: fromCentavos(Math.abs(acc.balance)),
          dueDate: `2026-03-${String(acc.payment_due_day!).padStart(2, '0')}`,
          source: 'credit_card',
          daysUntilDue: acc.payment_due_day!,
        })
      }
      // Subscriptions
      ...
      return {
        success: true,
        bills,
        summary: { count: bills.length, totalAmount: totalDue, daysAhead },
        message: `${bills.length} upcoming bill(s), total: $${totalDue.toFixed(2)}.`,
      }
    },
  }),
}
...
const SYSTEM_PROMPT = `You are Ivy, Shikin's AI financial assistant. You help users manage their personal finances.
...
async function runSuite() {
  ...
  console.log('╔══════════════════════════════════════════════════════════════════╗')
  console.log(
    `║  Shikin AI Test Suite — ${String(SCENARIOS.length).padEnd(2)} scenarios${' '.repeat(33)}║`
  )
...
async function runAdHoc(prompt: string) {
  ...
  console.log('╔══════════════════════════════════════════════════════════════════╗')
  console.log(`║  Shikin AI CLI Test (ad-hoc)${' '.repeat(36)}║`)
  console.log(`║  Provider: ${(provider + ' / ' + (modelId || 'default')).padEnd(52)}║`)
  console.log('╚══════════════════════════════════════════════════════════════════╝')
  console.log()
