...
async function createStatement(paidAmount = 0, balance = 1000, second = false) {
  ...
  return createCardStatement('card', draft, preview.revision)
}
...
describe('browser/Tauri card payment service on real SQLite 021', () => {
  it('creates, explicitly edits and deletes statement-only accounting without fake transactions', async () => {
    ...
    const deletePreview = await previewDeleteCardStatement(statement.id)
    await deleteCardStatement(
      statement.id,
      deletePreview.revision,
      'operator',
      'duplicate statement'
    )
  ...
  it('links report-excluded ordinary evidence, preserves money tables, unlinks, and retains immutable history after deletion', async () => {
    ...
    const linked = await linkCardPayment(input, preview.revision)
    ...
    const unlinkPreview = await previewUnlinkCardPayment(link.id)
    await unlinkCardPayment(link.id, unlinkPreview.revision, 'operator', 'wrong cycle')
    ...
    const deletePreview = await previewDeleteCardStatement(statement.id)
    await deleteCardStatement(statement.id, deletePreview.revision, 'operator', 'remove statement')
  ...
  it('keeps explicit attribution total stable and rejects unsafe paid/balance edits', async () => {
    ...
    const preview = await previewLinkCardPayment(input)
    expect(await linkCardPayment(input, preview.revision)).toMatchObject({
      paidAmount: 600,
    ...
    await expect(previewDeleteCardStatement(statement.id)).rejects.toThrow(/Unlink active/)
  })
  ...
  it('enforces canonical capacity across statements and rejects pending evidence', async () => {
    ...
    await linkCardPayment(firstInput, firstPreview.revision)
    await expect(
  ...
  it('preserves and diagnoses legacy overpayment without allowing it to grow', async () => {
    ...
    await updateCardStatement(statement.id, { note: 'retain history' }, harmless.revision)
  ...
  it('creates a real transfer and optional evidence link atomically, including rollback on link failure', async () => {
    ...
    const result = await recordCardPayment(input, preview.revision)
    ...
    const before = {
      accounts: db.prepare('SELECT * FROM accounts ORDER BY id').all(),
      transactions: db.prepare('SELECT * FROM transactions ORDER BY id').all(),
      links: db.prepare('SELECT * FROM card_statement_payment_links ORDER BY id').all(),
      statements: db.prepare('SELECT * FROM credit_card_statements ORDER BY id').all(),
      audit: db.prepare('SELECT * FROM audit_log ORDER BY id').all(),
      revision: db.prepare('SELECT * FROM app_data_state').all(),
    }
    await expect(recordCardPayment(failingInput, failingPreview.revision)).rejects.toThrow(
      /link failure/
    )
    expect({
      accounts: db.prepare('SELECT * FROM accounts ORDER BY id').all(),
  ...
  it('rejects stale reviewed plans and currency mismatches with zero writes', async () => {
    ...
    await expect(linkCardPayment(input, preview.revision)).rejects.toThrow(/changed after preview/i)
    expect(count('card_statement_payment_links')).toBe(0)
